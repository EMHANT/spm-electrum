...
void BattlePutTextOnWindow(const u8 *text, u8 windowId)
{
    ...
    printerTemplate.y = textInfo[windowId].y - 1;
...
void SetPpNumbersPaletteInMoveSelection(u32 battler)
{
    ...
    gPlttBufferUnfaded[BG_PLTT_ID(0) + 5] = palPtr[(var * 2) + 0];
    gPlttBufferUnfaded[BG_PLTT_ID(0) + 4] = palPtr[(var * 2) + 1];

    CpuCopy16(&gPlttBufferUnfaded[BG_PLTT_ID(0) + 5], &gPlttBufferFaded[BG_PLTT_ID(0) + 5], PLTT_SIZEOF(1));
    CpuCopy16(&gPlttBufferUnfaded[BG_PLTT_ID(0) + 4], &gPlttBufferFaded[BG_PLTT_ID(0) + 4], PLTT_SIZEOF(1));
}
